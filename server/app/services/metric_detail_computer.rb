class MetricDetailComputer
  ...
  # Pre-computed trend SQL fragments for PR metrics.
  ...
  def call
    @is_session ? compute_session_detail : compute_pr_detail
  end
  ...
  def compute_session_detail
    ...
    trend = compute_trend(current, current_start, now)
  ...
  def compute_trend(scope, from, to)
    if @is_session
      ...
      frags = PR_TREND_SELECTS[MetricsAggregator::PR_METRIC_COLUMNS[@slug]]
      rows = scope
        .select(frags[:bucket], frags[:avg], frags[:min], frags[:max], frags[:cnt])
        .group(frags[:group])
        .order(BUCKET_ORDER)
