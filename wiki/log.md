...
1. **Pinned `Stripe.api_version` to `2025-04-30.basil`** in `config/initializers/stripe.rb`. The stripe-ruby v19 default version had silently moved `current_period_start` / `current_period_end` off the `Subscription` object onto each subscription item, which caused `CheckoutCompleted` to crash with `NoMethodError`, leaving every paid upgrade stuck on `free`.
