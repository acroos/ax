...
Files with a "Code generated" header (`// Code generated by scripts/codegen-agents — DO NOT EDIT.` in Go/TS, `# Code generated ...` in Ruby) are emitted by `scripts/codegen-agents/generate.rb` from `config/agents.yaml`. Never edit them by hand — CI's `codegen-check` job runs `just codegen-agents-check` on every PR and fails if generated outputs differ from disk.

To update generated files:
1. Edit `config/agents.yaml`.
2. Run `just codegen-agents`.
3. Commit both `config/agents.yaml` and all changed `*.gen.*` files together.

**Optional pre-commit hook** (copy to `.git/hooks/pre-commit` if you want local enforcement — do not commit this file):
